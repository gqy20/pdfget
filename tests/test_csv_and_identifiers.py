...
class TestPMCIDNormalization(CSVTestMixin):
    """测试PMCID标准化功能（从 test_pmcid_csv.py 整合）"""

    def setup_method(self):
        """每个测试前的设置"""
        self.fetcher = PaperFetcher()

    def test_normalize_pmcid_with_valid_formats(self):
        """测试：标准化不同格式的 PMCID"""
        # 测试各种有效格式
        assert self.fetcher._normalize_pmcid("123456") == "PMC123456"
        assert self.fetcher._normalize_pmcid("PMC123456") == "PMC123456"
        assert self.fetcher._normalize_pmcid("  123456  ") == "PMC123456"
        assert self.fetcher._normalize_pmcid("\tPMC123456\n") == "PMC123456"

    def test_normalize_pmcid_with_invalid_formats(self):
        """测试：处理无效格式的 PMCID"""
        # 测试各种无效格式
        assert self.fetcher._normalize_pmcid("") == ""
        assert self.fetcher._normalize_pmcid("   ") == ""
        assert self.fetcher._normalize_pmcid("ABC123") == ""
        assert self.fetcher._normalize_pmcid("PM-123456") == ""
        assert self.fetcher._normalize_pmcid("123-456") == ""

    def test_read_pmcid_from_csv_simple(self, csv_file_with_pmcids):
        """测试：读取简单的 PMCID 列表（使用fixture）"""
        pmcid_list = self.fetcher._read_pmcid_from_csv(str(csv_file_with_pmcids))
        assert pmcid_list == ["PMC123456", "PMC789012", "PMC345678"]

    def test_read_pmcid_from_csv_with_header(self, csv_file_with_header):
        """测试：读取带标题行的 CSV 文件（使用fixture）"""
        pmcid_list = self.fetcher._read_pmcid_from_csv(str(csv_file_with_header))
        assert pmcid_list == ["PMC123456", "PMC789012"]

    def test_read_pmcid_from_csv_empty_file(self, temp_output_dir):
        """测试：读取空文件"""
        empty_file = temp_output_dir / "empty.csv"
        empty_file.write_text("")

        pmcid_list = self.fetcher._read_pmcid_from_csv(str(empty_file))
        assert pmcid_list == []

    def test_read_pmcid_from_csv_file_not_found(self):
        """测试：文件不存在时抛出异常"""
        with pytest.raises(FileNotFoundError):
            self.fetcher._read_pmcid_from_csv("nonexistent.csv")


class TestIdentifierDetection(CSVTestMixin):
    """测试标识符类型自动检测（从 test_identifier_download.py 整合）"""
